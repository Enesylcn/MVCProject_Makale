...
namespace MVCProject_Makale.Controllers
{
    ...
    public class NoteController : Controller
    {
        ...
        [HttpPost]
        public ActionResult GetLiked(int[] ids) //sayı dizisi
        {
            Kullanici user = (Kullanici)Session["login"];
            ...
            if (user!=null)
            {
                if (ids!=null)
                {
                    listid = ly.ListQueryable().Where(x => x.Kullanici.ID == user.ID && ids.Contains(x.Note.ID)).Select(x => x.Note.ID).ToList();
        ...
        public ActionResult SetLiked(int notid,bool liked) //sayı dizisi
        {
            ...
            return Json(new { hata = true, sonuc = not.LikeCount, mesaj="Beğeni Gerçekleşemedi." });
        }
